...
public class DateUtil {
	...
	public static Date getNextTime(int hour, int min) {
		...
		} else if (calendar.get(Calendar.HOUR_OF_DAY) == hour && calendar.get(Calendar.MINUTE) >= min) {
			calendar.add(Calendar.DATE, 1);
		}
	 ...
	 *            {String} 时间字串
	 * @param format
	 *            {String} 格式化标准 【exam:'yyyy-MM-dd hh:mm:ss'】
	 ...
	 *
	 * @param da1(起始日期
	 *            格式：yyyy yyyyMM yyyyMMdd)
	 * @param da2(截止日期)
	 * @return
	 */
	public static String getTimeBetweenDates(String da1, String da2) {
		String str = "";
		if (StringUtil.isEmpty(da1) || StringUtil.isEmpty(da2) || !StringUtil.isNumeric(da1)
				|| !StringUtil.isNumeric(da2) || "19000101".equals(da1)) {
			return "--";
		}
		Date now = new Date();
		int years = 0;
	 ...
	 *
	 * @param min
	 *            如果要获得前几分钟时间，该参数为负数； 如果要获得后几分钟时间，该参数为正数
	 ...
	 *
	 * @param second
	 *            如果要获得前几秒钟时间，该参数为负数； 如果要获得后几秒钟时间，该参数为正数
	...
	/**
	 * 获取当前时间，精确到秒 时间格式为yyyyMMddHHmmss
	 *
	 * @return String 精确到秒的当前时间
	 */
	...
	/**
	 * 获取当前时间，精确到毫秒 时间格式为yyyyMMddHHmmssSSS
	 *
	 * @return String 精确到毫秒的当前时间
	 */
	...
	/**
	 * 依据传入的时间格式获取当前时间 比如传入的是yyyyMMdd,则获取的是精确到天的当前时间
	 * 比如传入的是yyyyMMddHHmmss,则获取的是精确到秒的当前时间
	 *
	 * @param pattern
	 *            时间格式字符串
	 * @return String 符合要求的时间
	 */
	 ...
	 * TODO 获取当月的第一天或最后一天
	 *
	 * @author lsy 2017年2月28日 上午10:49:31
	 * @param a=0,b=1
	 *            当月第一天
	 * @param a=1,b=0
	 *            当月最后一天
